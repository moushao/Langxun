...
public class PraiseWidget extends TextView {
    ...
    public void setDatas(List<ThumbsUp> datas) {
        this.datas = datas;
    }
    ...
    private void createSpanStringBuilder(List<ThumbsUp> datas) {
        if (datas == null || datas.size() == 0)
            return;
        ...
        if (spanStrBuilder == null) {
            ...
            for (int i = 0; i < datas.size(); i++) {
                PraiseClick praiseClick = new PraiseClick.Builder(getContext(), datas.get(i))
                        ...
                        .build();
                try {
                    spanStrBuilder.append(datas.get(i).getLikeName(), praiseClick, 0);
