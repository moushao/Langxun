...
public class EaseChatFragment extends EaseBaseFragment implements EMMessageListener {
    ...
    protected ArrayList<String> meetingGroup;
    ...
    protected int[] itemStrings = {R.string.attach_take_pic, R.string.attach_picture, R.string.attach_video/*, R.string
            .attach_location*/};
    protected int[] itemdrawables = {R.drawable.paise, R.drawable.zhaopian, R.drawable.icon_video/*, R
            .drawable.weizhi*/};
    ...
    protected void onConversationInit() {
        ...
        if (!isRoaming) {
            ...
            fetchQueue.execute(new Runnable() {
                ...
                public void run() {
                    try {
                        EMClient.getInstance().chatManager().fetchHistoryMessages(toChatUsername, EaseCommonUtils
                                .getConversationType(chatType), pagesize, "");
                        final List<EMMessage> msgs = conversation.getAllMessages();
                        int msgCount = msgs != null ? msgs.size() : 0;
                        if (msgCount < conversation.getAllMsgCount() && msgCount < pagesize) {
                            String msgId = null;
            ...
            fetchQueue.execute(new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        EMClient.getInstance().chatManager().fetchHistoryMessages(toChatUsername, EaseCommonUtils
                                .getConversationType(chatType), pagesize, (messages != null && messages.size() > 0) ?
    ...
    public void onMessageReceived(List<EMMessage> messages) {
        for (EMMessage message : messages) {
            ...
            if (username.equals(toChatUsername) || message.getTo().equals(toChatUsername) || message.conversationId()
    ...
    private void sendOhterGroup(final EMMessage message) {
        int i = 0;
        for (final String meeting : meetingGroup) {
            i++;
            new Timer().schedule(new TimerTask() {
                @Override
                public void run() {
                    EMMessage nMessage = message;
                    nMessage.setChatType(ChatType.GroupChat);
                    nMessage.setTo(meeting);
                    EMClient.getInstance().chatManager().sendMessage(nMessage);
                }
            }, 500 * i);
        }
            ...
            getActivity().runOnUiThread(new Runnable() {
                ...
                public void run() {
                    if (toChatUsername.equals(groupId)) {
                       /* EventBus.getDefault().post(new EventMessage(EventMessage.GROUP_CHANGED, EventMessage
                                .GROUP_CHANGED));*/
