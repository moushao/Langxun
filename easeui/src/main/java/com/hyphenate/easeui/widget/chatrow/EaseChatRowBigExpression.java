    ...
    public void onSetUpView() {
        ...
        if (emojicon != null) {
            if (emojicon.getBigIcon() != 0) {
                ...
                //                Glide.with(activity).load(emojicon.getBigIcon()).placeholder(R.drawable
                // .ease_default_expression).into(imageView);

                Glide.with(context).load(emojicon.getBigIcon()).apply(new RequestOptions().diskCacheStrategy
                        (DiskCacheStrategy.ALL)
                        .placeholder(R
                                .drawable.ease_default_expression).error(R.drawable.ease_default_expression).priority
                                (Priority.HIGH)).into(imageView);
            } else if (emojicon.getBigIconPath() != null) {
                //                Glide.with(activity).load(emojicon.getBigIconPath()).placeholder(R.drawable
                // .ease_default_expression).into(imageView);

                Glide.with(context).load(emojicon.getBigIconPath()).apply(new RequestOptions().diskCacheStrategy
                        (DiskCacheStrategy.ALL)
